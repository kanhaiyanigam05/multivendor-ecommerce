...
export const CartModalProvider = ({ children }) => {
  ...
  const [total, setTotal] = useState(
    JSON.parse(localStorage.getItem("cart-total")) || { substotal: 0 }
  );
  const handleAddCartItem = ({ product = [], quantity = 1, subtotal = 0 }) => {
  ...
  useEffect(() => {
    ...
    let subtotal = data.reduce((acc, curr) => acc + curr.subtotal, 0);
    subtotal = parseFloat(subtotal).toFixed(2);
    setTotal({ subtotal: subtotal });
    localStorage.setItem("cart", JSON.stringify(data));
    localStorage.setItem("cart-total", JSON.stringify({ subtotal: subtotal }));
  }, [data]);
  return (
...
export const ShopingCart = () => {
  ...
  const getAllRelatedProducts = () => {
    // Map to improve performance by having quick access to products
    const productsMap = PRODUCTS_DATA.reduce((acc, product) => {
    ...
    data.forEach((product) => {
      const rProducts = product.related_products.map((relatedProductId) => {
        return productsMap[relatedProductId];
      });

      rProducts.forEach((rProduct) => {
        if (
          rProduct &&
          !allRelatedProducts.includes(rProduct) &&
          !data.some((item) => item.id === rProduct.id)
        ) {
          allRelatedProducts.push(rProduct);
        }
      });
    });

    // Sort related products by ID
    allRelatedProducts.sort((a, b) => a.id - b.id);
    setRelatedProducts(allRelatedProducts);
  };

  const handleCartAdd = (productId) => {
    const selectedProduct = relatedProducts.find((p) => p.id === productId);
    handleAddCartItem({
      product: selectedProduct,
      subtotal: (selectedProduct.discounted_price
                ...
                <div className="list-cart">
                  {relatedProducts.length > 0 &&
                    relatedProducts.map((product, i) => {
                      return (
                        <div className="list-cart-item" key={i}>
                          <div className="image">
                            <img
                              className="lazyload"
                              data-src={product.first_image}
              ...
              <div className="tf-mini-cart-wrap">
                ...
                <div className="tf-mini-cart-bottom">
                  <div className="tf-mini-cart-tool">
                    <div
                      ...
                      onClick={() => setSubModal({ ...subModal, note: true })}
                    >
                      <svg
                        width="21"
                        height="20"
                    ...
                    <div
                      ...
                      onClick={() =>
                        setSubModal({ ...subModal, shipping: true })
                      }
                    >
                      <svg
                        width="20"
                    ...
                    <div
                      ...
                      onClick={() => setSubModal({ ...subModal, gift: true })}
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="17"
                      ...
                      <div className="text-caption-1">Gift</div>
                    </div>
                    <div
                      className="tf-mini-cart-tool-btn btn-add-coupon"
                      onClick={() => setSubModal({ ...subModal, coupen: true })}
                    >
                      <svg
                        width="21"
                        height="20"
                  ...
                  <div className="tf-mini-cart-bottom-wrap">
                    <div className="tf-cart-totals-discounts">
                      <h5>Subtotal</h5>
                      <h5 className="tf-totals-total-value">{`$${total.subtotal}`}</h5>
                    </div>
                    <div className="tf-cart-checkbox">
                      ...
                      <label htmlFor="CartDrawer-Form_agree">
                        I agree with
                        <a href="term-of-use.php" title="Terms of Service">
                          Terms & Conditions
                        </a>
                      </label>
                    </div>
                    ...
                    <form className="form-add-note tf-mini-cart-tool-wrap">
                      ...
                      <div className="tf-cart-tool-btns">
                        ...
                        <div
                          ...
                          onClick={() =>
                            setSubModal({ ...subModal, note: false })
                          }
                        >
                          Cancel
                        </div>
                ...
                >
                  <div className="tf-mini-cart-tool-content">
                    ...
                    <form className="form-estimate-shipping tf-mini-cart-tool-wrap">
                      <div className="mb_12">
                      ...
                      <div className="tf-cart-tool-btns">
                        ...
                        <div
                          className="text-center w-100 text-btn-uppercase tf-mini-cart-tool-close"
                          onClick={() =>
                            setSubModal({ ...subModal, shipping: false })
                          }
                        >
                          Cancel
                        </div>
                ...
                >
                  <form className="tf-product-form-addgift">
                    <div className="tf-mini-cart-tool-content">
                      ...
                      <div className="tf-cart-tool-btns tf-mini-cart-tool-wrap">
                        <button type="submit" className="btn-style-2 w-100">
                          <span className="text text-btn-uppercase">
                            Add a gift wrap
                          </span>
                        ...
                        <div
                          className="text-center w-100 text-btn-uppercase tf-mini-cart-tool-close"
                          onClick={() =>
                            setSubModal({ ...subModal, gift: false })
                          }
                        >
                          Cancel
                        </div>
                    ...
                    <form className="form-add-coupon tf-mini-cart-tool-wrap">
                      ...
                      <div className="tf-cart-tool-btns">
                        ...
                        <div
                          className="text-center w-100 text-btn-uppercase tf-mini-cart-tool-close"
                          onClick={() =>
                            setSubModal({ ...subModal, coupen: false })
                          }
                        >
                          Cancel
                        </div>
