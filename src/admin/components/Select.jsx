...
const Select = ({
  ...
  return (
    <div className={`flex max-w-[100%] w-full flex-col gap-1 ${wrapperClass}`}>
      {label && (
        <label
          htmlFor={id}
      ...
      {variant === "react-select" ? (
      ...
      ) : variant === "advance" || variant === "simple" ? (
        <div className="relative" ref={wrapperRef}>
          ...
          <Input
            ...
            onChange={(e) =>
              searchable
                ...
                : !multiple && setSelectValue(e.target.value)
            }
          />
                ...
                >
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      setSelectValue(
                        selectValue.filter((label) => label !== item)
                      );
                    }}
                  >
          ...
          <ul
            className={`${
              !focus ? "hidden " : ""
            }px-1 py-2 absolute z-10 bg-white border border-gray-200 rounded-lg w-full max-h-[250px] overflow-y-auto mt-2 shadow-lg unique-dropdown`}
          >
            {options
              .filter((item) =>
              ...
              .map((item, i) => {
                const isChecked = multiple
                  ? (selectValue || []).includes(item.label)
                  : selectValue === item.label;

                const handleItemChange = () => {
                  if (multiple) {
                    const newValues = isChecked
                      ? selectValue.filter((label) => label !== item.label)
                      : [...(selectValue || []), item.label];
        ...
        <select
          id={id}
          className={`h-[35px] bg-gray-50 rounded-lg w-full text-[#303030] text-[13px] border py-1 px-2 outline-offset-2 focus-within:outline-2 focus-within:outline-blue-800 ${className}${
